...
import { PremiumBadge } from '@/components/form/premium-badge'
import { Icons } from '@/lib/constants/icons'
import { deleteTemplate, favoriteTemplate, useTemplate } from '@/lib/data/templates/actions'
import type { Template } from '@/lib/data/templates/schemas'
import type { FilterSearchParams } from '@/lib/data/templates/types'
import { generateTemplateUrl } from '@/lib/data/templates/utils'
import type { User } from '@/lib/data/users/schemas'
        ...
        <CardContent>
          ...
          <div className="text-muted-foreground flex flex-wrap gap-2 text-sm">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-foreground"> {formatLargeNumber(template.useCount)}</span> use
              {template.useCount === 1 ? '' : 's'}
            </div>
            <div className="flex items-center gap-1.5">
              <Icons.heart className="size-3.5" />
              <span className="text-foreground"> {formatLargeNumber(template.favoriteCount)}</span> favorite
              {template.favoriteCount === 1 ? '' : 's'}
            </div>
          </div>
          <ul className="mt-2 flex flex-wrap gap-1">
            {template.tags.map((tag, idx) => (
              <li key={idx}>
                <Badge variant={filters.tags?.includes(tag) ? 'primary' : 'secondary'}>#{tag}</Badge>
              </li>
            ))}
          </ul>
        </CardContent>
